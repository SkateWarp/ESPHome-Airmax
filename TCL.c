/**
* Created by Miguel Ángel López for Haiwer A/C on 20/07/19
* Modified for TCL by xaxexa
* Modified for AirMax brand A/C by SkateWarp
***/
...
#define MODE_AUTO      	0b00010101
#define MODE_COOL       0b00010001
#define MODE_DRY      	0b00010011
#define MODE_FAN_ONLY   0b00010010
#define MODE_HEAT       0b00010100
...
#define SWING_MODE_MASK    	0b01000000
...
public:
    ...
    void loop() override  {
        ...
        if (Serial.available() > 0) {
			...
			String raw = getHex(dataRX, sizeof(dataRX));
			ESP_LOGV("TCL", "Complete RX : %s ", raw.c_str());

			if (check != dataRX[60]) {
				ESP_LOGV("TCL", "Invalid checksum %x", check);
				return;
			} else {
				ESP_LOGV("TCL", "Checksum OK %x", check);
			}
...
// Climate control
    ...
    void sendData(byte * message, byte size) {
        ...
        String raw = getHex(message, size);
        ESP_LOGV("TCL", "Sended message: %s ", raw.c_str());
